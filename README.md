...
After the pipeline run, (re)generate the bridge CSVs and open the Excel dashboard:

```bash
.venv/bin/python workspaces/GetFinance-Show/scripts/report/export_bridge.py
.venv/bin/python workspaces/GetFinance-Show/scripts/report/build_report.py
```

Then open:

```
workspaces/GetFinance-Show/report/GetFinance-Show-Report.xlsx
```

The committed workbook already displays synthetic data. The bridge CSVs (gitignored) are
regenerated by the two commands above; the Power Query dashboard reads them automatically.
